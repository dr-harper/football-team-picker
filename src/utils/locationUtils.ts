...
export const getPlacesBasedOnLocation = async (): Promise<{ location: string; places: string[] }> => {
    try {
        ...
        // Default to Generic UK places
        return { location: 'Generic UK', places: teamPlaces['Generic UK'].places };
    } catch (error) {
        console.error('Failed to get location:', error);
        return { location: 'Generic UK', places: teamPlaces['Generic UK'].places }; // Default to Generic UK places if location fails
    }
};
